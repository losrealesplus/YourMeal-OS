/**
 * RELEASE-CROSSFLOW · C1 Kitchen → Delivery operable.
 *
 * Certifies FLOW-01 handoff remains available for the cross-flow chain:
 *   FLOW01_T1…T4 STARTED/COMPLETED (Kitchen → Delivery FULL PASS)
 *
 * Does NOT re-run domain mutations. Does NOT open C2/C3/C4.
 * Binds to tag `flow01-pass` + Flow runner contract on main.
 */
...
export const RELEASE_CROSSFLOW_C1_MAPPED_TOKENS = Object.freeze([
 ...
 * @returns {C1Result}
 */
export function runReleaseCrossflowC1KitchenDelivery(opts = {}) {
  ...
  if (!pkg?.scripts?.["test:flow01-canonical"]) {
    return {
      ...
      reason: [
        ...
        "RELEASE-CROSSFLOW C1 requires the FLOW-01 runner on main.",
      ].join("\n"),
    };
  }
  checks.push("flow01_canonical_script_present");

  const expected = [...RELEASE_CROSSFLOW_C1_MAPPED_TOKENS];
  if (
  ...
  try {
    ...
    return {
      ...
      reason: [
        ...
        "RELEASE-CROSSFLOW C1 reuses FLOW-01 certification — tag required.",
      ].join("\n"),
    };
  }
  ...
  return {
    ...
    source: "flow01-pass + FLOW-01 T1…T4 pipeline (no C2+ · no domain re-run)",
  };
}
