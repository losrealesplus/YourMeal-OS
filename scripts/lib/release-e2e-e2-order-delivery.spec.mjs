/**
 * Unit tests for RELEASE-E2E E2 Order → Delivery.
 */
...
import {
  RELEASE_E2E_E2_MAPPED_TOKENS,
  runReleaseE2eE2OrderDelivery,
} from "./release-e2e-e2-order-delivery.mjs";

function tempGitRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "release-e2e-e2-"));
...
describe("release-e2e-e2-order-delivery", () => {
  it("maps FLOW-01 T1…T4 tokens", () => {
    assert.equal(RELEASE_E2E_E2_MAPPED_TOKENS.length, 8);
    assert.equal(RELEASE_E2E_E2_MAPPED_TOKENS[0], "FLOW01_T1_STARTED");
    assert.equal(RELEASE_E2E_E2_MAPPED_TOKENS[7], "FLOW01_T4_COMPLETED");
  });
  ...
  it("PASS when FLOW-01 script, tag, acta, and spec exist", () => {
    ...
    const r = runReleaseE2eE2OrderDelivery({ cwd });
