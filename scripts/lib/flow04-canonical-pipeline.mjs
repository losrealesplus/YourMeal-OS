/**
 * FLOW-04 canonical pipeline validator (Inventory Consumption).
 * Contract frozen in docs/00-status/FLOW_04_INVENTORY_CONSUMPTION_SPEC.md.
 * Evidence before Implementation — runner exists before domain code.
 */

export const FLOW04_CANONICAL_STEPS = Object.freeze([
  "FLOW04_T1_STARTED",
  "FLOW04_T1_COMPLETED",
  "FLOW04_T2_STARTED",
  "FLOW04_T2_COMPLETED",
...
export const FLOW04_EXIT = Object.freeze({
  PASS: 0,
  FAIL: 1,
  BLOCKED: 2,
});
 ...
 */
export function listFlow04OutOfOrderSteps(observed) {
  const filtered = observed.filter((s) => FLOW04_CANONICAL_STEPS.includes(s));
  const outOfOrder = [];
  for (let i = 0; i < filtered.length; i++) {
    if (filtered[i] !== FLOW04_CANONICAL_STEPS[i]) {
...
/**
 * Validate FLOW-04 contract: every canonical step exactly once, in order.
 * @param {readonly string[]} observed
 */
export function validateFlow04Pipeline(observed) {
  ...
  const table = FLOW04_CANONICAL_STEPS.map((step) => ({
    step,
    expected: true,
    observed: (counts.get(step) ?? 0) === 1,
  }));

  const missing = FLOW04_CANONICAL_STEPS.filter(
    (step) => (counts.get(step) ?? 0) !== 1,
  );

  const out_of_order = listFlow04OutOfOrderSteps(observed);
  const unexpectedOrder = out_of_order.length > 0;
  const extras = observed.filter((s) => !FLOW04_CANONICAL_STEPS.includes(s));
  ...
  const ok =
    ...
    !unexpectedOrder &&
    extras.length === 0 &&
    observed.length === FLOW04_CANONICAL_STEPS.length;
 ...
 */
export function computeFlow04Durations(stepTimestamps) {
  const t = (step) =>
    typeof stepTimestamps[step] === "number" ? stepTimestamps[step] : null;
  const span = (a, b) => (a != null && b != null && b >= a ? b - a : null);

  return {
    t1_ms: span(t("FLOW04_T1_STARTED"), t("FLOW04_T1_COMPLETED")),
    t2_ms: span(t("FLOW04_T2_STARTED"), t("FLOW04_T2_COMPLETED")),
    t3_ms: span(t("FLOW04_T3_STARTED"), t("FLOW04_T3_COMPLETED")),
    flow_total_ms: span(t("FLOW04_T1_STARTED"), t("FLOW04_T3_COMPLETED")),
  };
}
 ...
 */
export function buildFlow04EvidenceReport({
  status,
  reason = "",
  pipeline = [],
  validation = null,
  ...
  return {
    ...
    reason: reason || "",
    gate: "FLOW-04",
    contract: "FLOW_04_INVENTORY_CONSUMPTION_SPEC",
    principle: "Evidence before Implementation",
    code_status,
    at: new Date().toISOString(),
    pipeline: [...pipeline],
    expected: [...FLOW04_CANONICAL_STEPS],
    ...
    terminal: terminal ?? {
      consumption_status: fullPass ? "sealed" : null,
    },
...
export function formatFlow04ComparisonTable(result) {
  const lines = [
...
/** Extract ordered FLOW-04 step names from console / log lines (canonical only). */
export function extractFlow04Steps(consoleLines) {
  const steps = [];
  for (const line of consoleLines) {
    const text = typeof line === "string" ? line : String(line);
    const m = text.match(
      /\[FLOW-04\]\s+(FLOW04_T[1-3]_(?:STARTED|COMPLETED))/,
    );
    if (m && FLOW04_CANONICAL_STEPS.includes(m[1])) steps.push(m[1]);
  }
  return steps;
}

/** @param {1|2|3} n */
export function flow04StepsThrough(n) {
  return FLOW04_CANONICAL_STEPS.slice(0, n * 2);
}
...
export function certifiedThroughTransition(observed) {
  ...
  let through = 0;
  for (let n = 1; n <= 3; n++) {
    const started = `FLOW04_T${n}_STARTED`;
    const completed = `FLOW04_T${n}_COMPLETED`;
...
/**
 * Progressive evaluation for incremental deliveries FLOW04-001..003.
 *
 ...
 */
export function evaluateFlow04Progress(observed, opts = {}) {
  ...
  const next_step =
    certified_through >= 3
      ? null
      : (FLOW04_CANONICAL_STEPS[certified_through * 2] ?? "FLOW04_T1_STARTED");
  ...
  if (duplicates.length || out_of_order.length || extras.length) {
    return {
      ...base,
      status: "FAIL",
      reason: "Pipeline violates FLOW04_* contract (duplicate / order / extras)",
      ok: false,
      firstFailure: duplicates[0] ?? out_of_order[0] ?? extras[0] ?? null,
      delivery: through ? `FLOW04-00${through}` : "FLOW-04",
  ...
  if (observed.length === 0) {
    return {
      ...
      status: "BLOCKED",
      reason:
        "FLOW-04 domain not implemented (runner ready; Evidence before Implementation)",
      ok: false,
      missing: [],
      firstFailure: "FLOW04_T1_STARTED",
      delivery: through ? `FLOW04-00${through}` : "FLOW-04",
  ...
  if (through) {
    if (missingExpected.length === 0) {
      return {
        ...
        reason:
          through < 3
            ? `PASS through T${through} · BLOCKED at ${FLOW04_CANONICAL_STEPS[through * 2]} for full FLOW-04`
            : "FLOW-04 complete",
        ok: true,
        missing: [],
        firstFailure: null,
        ...
        next_step:
          through < 3 ? FLOW04_CANONICAL_STEPS[through * 2] : null,
        delivery: `FLOW04-00${through}`,
    ...
    return {
      ...base,
      status: "FAIL",
      reason: `Delivery FLOW04-00${through} incomplete · missing ${missingExpected[0]}`,
      ok: false,
  ...
  if (certified_through > 0) {
    return {
      ...
      ok: false,
      missing: FLOW04_CANONICAL_STEPS.filter((s) => (counts.get(s) ?? 0) !== 1),
      firstFailure: next_step,
      delivery: `FLOW04-00${certified_through}`,
      delivery_status: "PASS",
      flow_status: "BLOCKED",
