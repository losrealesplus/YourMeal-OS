# Acta G-01 — plantilla

```markdown
# Gate G-01 — Operational Readiness

**Fecha:** …  
**Package:** enlaces a RC · FOV · FER · KUR · ECR · Open Risks · Known Limitations  
**Resultado:** APPROVED · APPROVED WITH CONDITIONS · REJECTED

---

## Integridad del Package

| # | Componente | ✅/❌ | Ref |
|---|------------|------|-----|
| 1 | Operational Model RC | | |
| 2 | FOV Report | | |
| 3 | FER | | |
| 4 | Knowledge Update (KUR / null) | | |
| 5 | Economic Confirmation Report | | |
| 6 | Open Risks | | |
| 7 | Known Limitations | | |

---

## Criterios de conocimiento

| Criterio | ✅/❌ | Evidencia |
|----------|------|-----------|
| Operational Validation | | |
| IOV-001 | | IVR-001 |
| IOV-002 | | IVR-002 |
| IOV-003 | | IVR-003 |
| Findings críticos cerrados | | |
| FOV ejecutado | | FO-… |
| FER cerrado | | FER-… |
| KU cerrado | | KUR-… |
| EC dictamen | | ECR-… |
| Riesgos Critical = 0 | | Open Risks |

---

## Condiciones (solo si APPROVED WITH CONDITIONS)

| ID | Condición | Owner | Fecha límite | Alcance Stage 2 afectado |
|----|-----------|-------|--------------|--------------------------|
| C-… | … | … | YYYY-MM-DD | … |

---

## Declaración

Este gate **no** aprueba código.  
Certifica (o rechaza) conocimiento suficiente para justificar Product Engineering.

**Resultado:** …

## Firmas / sesión
…
```

---

## Índice de actas

| ID | Fecha | Resultado | Estado |
|----|-------|-----------|--------|
| — | | | ⏸ |

---

## Relacionado

- [02 Decision](./02-decision.md)  
- [reports/](../reports/README.md)
