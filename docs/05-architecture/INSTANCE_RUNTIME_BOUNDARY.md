...
5. **Aislamiento de Administrador de Plataforma vs Administrador de Tenant (`Platform Admin != Tenant Admin`):**
   * **SaaS Administrator (`saas_admin`):** Administrador de la plataforma YourMeal OS con `tenant_id = null`. Posee la capability `saas.manage` para gobernar tenants globales y auditar la infraestructura SaaS desde `/saas`.
   * **Tenant Administrator (`company_admin`):** Administrador operativo exclusivo del tenant con `tenant_id = <UUID_DEL_TENANT>` y membresía aprobada en `tenant_members` (`status = 'approved'`). Posee capabilities operativas (`orders.manage`, `kitchen.operate`, `menus.write`, `admin.settings`) en su espacio `/admin`, pero tiene estrictamente prohibido el acceso a `/saas` y carece de facultades sobre otros tenants.
