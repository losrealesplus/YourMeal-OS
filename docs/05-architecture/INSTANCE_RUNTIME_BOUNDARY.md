...
5. **Aislamiento de Administrador de Plataforma vs Administrador de Tenant (`Platform Admin != Tenant Admin`):**
   * **Platform Principal (`saas_admin`):** Administrador y operador global de YourMeal OS con `tenant_id = null`. Posee facultades de gobierno de plataforma (`saas.manage`, `tenants.manage`, `onboarding.manage`, `records.purge`) y capacidad de supervisión e intervención operativa transversal sobre cualquier contexto de tenant explícitamente seleccionado y auditado. *Invariante canónico: "Platform Principal may operate an explicitly selected tenant context without becoming a tenant-scoped administrator."*
   * **Scoped Tenant Administrator (`company_admin`):** Administrador operativo exclusivo de un único tenant (`tenant_id = <UUID_DEL_TENANT>` y membresía aprobada en `tenant_members`). Posee capacidades de gestión operativa (`orders.manage`, `kitchen.operate`, `menus.write`, `admin.settings`) en su espacio `/admin` sobre los datos de su propio tenant, pero tiene estrictamente prohibido el acceso a `/saas`, no posee autoridad de onboarding de plataforma y carece de facultades cross-tenant. *Invariante canónico: "Tenant Admin is permanently restricted to its assigned tenant."*
   * **Autoridad del Motor de Onboarding (`Onboarding Engine Authority`):** El motor de onboarding y migración de datos pertenece a la plataforma (`saas_admin`). El SaaS Admin selecciona el tenant destino, carga los archivos de ingesta (Excel/PDF), revisa el mapeo semántico y autoriza la importación en la base de datos del tenant. El Tenant Admin opera los datos ya consolidados sin controlar el pipeline de ingesta de la plataforma.
