...
|----|----------|-----------|------|--------|-------|
| **FCR-001** | Ajustes de Tenant Surface muestran el mismo hub para quien también opera Platform — separación insuficiente Tenant vs Platform Surface | **P1** | Superficies | **Corrected** · READY FOR RE-CERTIFICATION | EP-OPS-002 Correction: hub = Tenant business settings; Platform = `/saas`. Ver [RBAC_SURFACE_CERTIFICATION](./RBAC_SURFACE_CERTIFICATION.md) |
...
| **FCR-004** | Workspace Entry Kitchen: esperado `/admin/kitchen-execution`; código → `/admin/kitchen` | **P2** | Workspace Entry | **Corrected** · READY FOR RE-CERTIFICATION | EP-OPS-002: canónico = `/admin/kitchen`; execution = pantalla secundaria |
| **FCR-005** | Workspace Entry Support / Accounting → hoy `/admin` | **P2** | Workspace Entry | **Corrected** · READY FOR RE-CERTIFICATION | EP-OPS-002: `/admin/support` · `/admin/accounting` |
| **FCR-006** | SaaS Admin Bootstrap aterriza en Tenant Surface (`/admin`); política Entry → `/saas` | ⚠ | Workspace Entry | **Corrected** · READY FOR RE-CERTIFICATION | Puro `saas_admin` → `/saas`; híbrido Bootstrap → tenant-first `/admin` |
