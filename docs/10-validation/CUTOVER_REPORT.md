...
```bash
rg -n 'cbeegcxkayybfncnuirg' --glob '!.git/**'
```

Expected: **no matches** in tracked binding files. Historical mentions may remain only inside this report’s “removed dependency” narrative if any; binding files must be clean.
